...
const CRPFWFlow = ({ getKeyValue, getArray }) => {
  const kv = (key, fallback) => (getKeyValue ? getKeyValue("CRPFWFlow", key, fallback) : fallback);
  const arr = (key, fallback) => (getArray ? getArray("CRPFWFlow", key, fallback) : fallback);

  const titleStart = kv("titleStart", "Process");
  const titleSpan = kv("titleSpan", "Flow");
  const subtitle = kv("subtitle", "Academic Excellence & Professional Growth");
  const steps = arr("stepsList", defaultSteps);
    ...
    <section className="PGPMFLEX-info-section pb-5">
      <Container>
        ...
        >
          ...
          <p className="PGPMFLEX-info-subtitle text-left mx-0">{subtitle}</p>
        </motion.div>
        ...
        <div className="PGPMFLEX-info-grid">
          {steps.map((step, index) => (
            <motion.div
              key={index}
              className="PGPMFLEX-info-card h-100"
              style={{
            ...
            >
              <div className="PGPMFLEX-info-icon-box flex items-center justify-center">
                <DynamicIcon name={step.icon || "HelpCircle"} size={32} />
              </div>
