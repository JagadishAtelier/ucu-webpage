...
function ConsotiumPillars({ getKeyValue, getArray }) {
  const navigate = useNavigate();

  const kv = (key, fallback) => (getKeyValue ? getKeyValue("ConsotiumPillars", key, fallback) : fallback);
  const arr = (key, fallback) => (getArray ? getArray("ConsotiumPillars", key, fallback) : fallback);

  const mainHeading = kv("mainHeading", "Our network is built on these four foundational pillars :");
  const pillars = arr("pillarsData", defaultPillars);
  const buttonText = kv("buttonText", "Offerings");
